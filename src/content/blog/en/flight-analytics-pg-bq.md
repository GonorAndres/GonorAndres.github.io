...
description: "A deep dive into building production-grade SQL analytics on real airline data, migrating to BigQuery via Python ETL, and the honest trade-offs between both systems -- with real timing, real costs, and real query plans."
date: "2026-03-18"
...
## Phase 2: PostgreSQL internals -- understanding the engine
...
**EXPLAIN ANALYZE** showed that a simple filter on `flights WHERE departure_airport = 'SVO' AND status = 'Arrived'` scanned all 65,664 rows (Sequential Scan, 33.9ms) until I created a composite index on `(departure_airport, status)`. The same query then used a Bitmap Index Scan and finished in 2.6ms -- a **13x improvement** from a single `CREATE INDEX` statement.

**Materialized views** delivered the most dramatic result: a route delay summary that took 174ms from raw tables served the same data in 0.13ms from the materialized view -- **1,300x faster**. The trade-off is staleness: you must `REFRESH MATERIALIZED VIEW CONCURRENTLY` to update the data without blocking reads.

**Partitioning** demonstrated how range-partitioning the flights table by month enables partition pruning: a query for July 2017 flights scans only the July partition instead of the entire table. The EXPLAIN output explicitly shows "Partitions selected: 1 of 5."

**VACUUM tuning** showed the mechanics of MVCC: after updating 50,000 rows, the table had 50,000 dead tuples consuming space. Standard VACUUM marks that space as reusable (but doesn't shrink the file); VACUUM FULL rewrites the table to reclaim disk space but takes an exclusive lock.

**WAL and checkpoints** revealed that a bulk operation generating 100,000 inserts and 100,000 updates produces measurable WAL volume, and that `synchronous_commit = off` can speed up writes at the cost of losing the last ~600ms of commits on a crash -- acceptable for analytics, never for financial transactions.
...
## Phase 3: Migration to BigQuery -- the pipeline
...
## Phase 4: PostgreSQL vs BigQuery -- the honest comparison
...
Airport coordinates enabled a geospatial layer that demonstrates both systems' GIS capabilities. In PostgreSQL, I wrote a PL/pgSQL haversine function to calculate great-circle distances from the `point` type. In BigQuery, the same calculation uses `ST_GEOGPOINT()` and `ST_DISTANCE()` -- built-in, no custom functions needed.

The distance analysis revealed that delay rates don't correlate linearly with route length. Short routes (<500km) and very long routes (4,000km+) have similar delay percentages. The highest delays cluster in the 500-2,000km range -- medium-haul routes where turnaround pressure is highest.

A Jupyter notebook ties everything together with interactive plotly charts (delay heatmaps, revenue Pareto curves, load factor rankings, before/after optimization comparisons) and folium maps showing the route network colored by delay severity. The route map is the "hero image" of the project: 104 airports connected by lines that shift from green to red as delays increase.
