...
**No double-dash em-dashes. Ever.** Never write `--` as punctuation in any content: blog posts, descriptions, titles, or comments. This is a known AI writing pattern and reads unnaturally. Instead:
- Use a semicolon when two related clauses could stand alone: "The model converges; the residuals do not."
- Use a colon to introduce: "The tricky part: the data had no consistent date format."
- Use a comma for soft pauses: "It worked, eventually."
- Restructure the sentence if none of the above fit.
This rule applies in both Spanish and English.
...
- Any sentence using `--` as punctuation
...
- SIMA: actuarial modeling platform (Lee-Carter, reserves, RCS), deployed on GCP
- GMM Explorer: medical expense insurance pricing with CNSF data, deployed on Vercel
